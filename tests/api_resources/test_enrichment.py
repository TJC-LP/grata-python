# File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

from __future__ import annotations

import os
from typing import Any, cast

import pytest

from grata import Grata, AsyncGrata
from grata.types import CompanyDetailed
from tests.utils import assert_matches_type

base_url = os.environ.get("TEST_API_BASE_URL", "http://127.0.0.1:4010")


class TestEnrichment:
    parametrize = pytest.mark.parametrize("client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    def test_method_create(self, client: Grata) -> None:
        enrichment = client.enrichment.create()
        assert_matches_type(CompanyDetailed, enrichment, path=["response"])

    @parametrize
    def test_method_create_with_all_params(self, client: Grata) -> None:
        enrichment = client.enrichment.create(
            company_uid="26AJRCR2",
            domain="slack.com",
        )
        assert_matches_type(CompanyDetailed, enrichment, path=["response"])

    @parametrize
    def test_raw_response_create(self, client: Grata) -> None:
        response = client.enrichment.with_raw_response.create()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        enrichment = response.parse()
        assert_matches_type(CompanyDetailed, enrichment, path=["response"])

    @parametrize
    def test_streaming_response_create(self, client: Grata) -> None:
        with client.enrichment.with_streaming_response.create() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            enrichment = response.parse()
            assert_matches_type(CompanyDetailed, enrichment, path=["response"])

        assert cast(Any, response.is_closed) is True


class TestAsyncEnrichment:
    parametrize = pytest.mark.parametrize("async_client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    async def test_method_create(self, async_client: AsyncGrata) -> None:
        enrichment = await async_client.enrichment.create()
        assert_matches_type(CompanyDetailed, enrichment, path=["response"])

    @parametrize
    async def test_method_create_with_all_params(self, async_client: AsyncGrata) -> None:
        enrichment = await async_client.enrichment.create(
            company_uid="26AJRCR2",
            domain="slack.com",
        )
        assert_matches_type(CompanyDetailed, enrichment, path=["response"])

    @parametrize
    async def test_raw_response_create(self, async_client: AsyncGrata) -> None:
        response = await async_client.enrichment.with_raw_response.create()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        enrichment = await response.parse()
        assert_matches_type(CompanyDetailed, enrichment, path=["response"])

    @parametrize
    async def test_streaming_response_create(self, async_client: AsyncGrata) -> None:
        async with async_client.enrichment.with_streaming_response.create() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            enrichment = await response.parse()
            assert_matches_type(CompanyDetailed, enrichment, path=["response"])

        assert cast(Any, response.is_closed) is True
